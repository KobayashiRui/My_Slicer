...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; ; i++)
  {
    ...
    float z = sqrt(1 / (1 + (pow(z_vec, 2) / pow(x_vec, 2))));
    float x = -1 * z_vec * z / x_vec;
    ...
    std::cout << "End Make Plane" << std::endl;
    //line segumentの作成
    //交点の計算(線分を求める)
    ...
    std::cout << "END line : " << line_datas.size() << std::endl;
...
void make_polygon2(std::vector<line> &line_datas, std::vector<point> &polygon)
{
  //とりあえず先頭の点データを取得
  bool end_flag = false;
  std::cout << "make poly: " << line_datas.size() << std::endl;
  point search_p = line_datas[0].p1;
  point goal_p = line_datas[0].p2;
  //先頭のlineデータを削除
  polygon.push_back(search_p);
...
void make_offset(std::vector<std::vector<point>> &polygons_data, std::vector<std::vector<point>> &result_polygons_data)
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < solution.size(); i++)
    {
      ...
      {
        polygon.push_back(point((float)solution[i][j].X, (float)solution[i][j].Y, (float)z_data)); //TODO サイズをここで戻す?
      }
